...
class GeneratorCommand(sublime_plugin.TextCommand):
	...
	def run(self, edit):
		...
		while True :
			...
			self.upadateAutotasks(edit,task.end()+1,read,write)			
	...
	def rw_vartranfer(self,rw,offs,vname,bml):
		...
		var_proto={};
		var_proto['r']="\t{vname}[{v[1]}:{v[0]}]=idata[{i[1]}:{i[0]}];";
		var_proto['w']="\todata[{i[1]}:{i[0]}]={vname}[{v[1]}:{v[0]}];";
		def cidxrng(rng):
		...
		return var_proto[rw].format(**locals())			
